...
\citep[][extended by \citealt{2000ApJ...543..686P}]{1992ApJS...80..753S} to our own ionization and
 ...
 (typically 1000s of simulation. time) with heating and cooling rates applied using the optically 
 thin equations suggested by \cite{1994ApJ...435..756B}. The final state of the simulation
 ...
 state of the gas, and hence the true heating and cooling rates based upon the attenuation
 between the radiation source and each cell in the simulation. The detailed nature of the RT allows
 all mechanisms of frequency dependant radiation attenuation (e.g. photoionization absorption and
 line scattering) to be fully dealt with. Also of significance is the effect of multiple scattering, which
 is fully included in the Monte-Carlo method of RT, and can be important where sight-lines from a 
 cell back to the centre are heavily obscured. The cycle is 
 then completed by \textsc{python} passing back the improved estimates of heating and cooling 
 rates to \textsc{zeus}, where they are used to provide `correction' factors to the optically thin 
...
type spectrum, $\rm{L(\nu)\propto \exp{(-h\nu/k_BT_x)}}$  with $\rm{T_x=5.6\times10^7K}$, 
...
developing wind along the sightline back to the central source.
\textcolor{red}{This material not only attenuates the radiation but
can also significantly change the 
SED seen in a given cell which in turn dramatically changes the stability curve 
on a cell by cell basis
\citep{2017MNRAS.467.4161D}.}
We
...
within about $\rm{0.2~R_{IC}}$ a turbulent clump of subsonic gas forms, 
...
($\dot{\rm{M}}_{\rm{wind,outer}}$) listed in
Table \ref{table:wind_param}   
...
\textcolor{red}{We have also compared our mass-loss rates from the 
disk surface with those reported
by \cite{1996ApJ...461..767W}. We find a somewhat smaller mass-loss
rate per unit area of the disk for the same central source luminosity than implied 
by their equation 4.8. We also see
a faster drop off with radius, although we have a much reduced radial
dynamic range because we truncate our disk at $\rm{R=2R_{IC}}$. 
Both of these effects are due to a reduction in radiative flux in the acceleration 
zone due to attenuation. A reduction in mass-loss rate of about a factor of 5 
was also seen by H18 when RT effects were first introduced into these simulations.}
...
at lower luminosities, \textcolor{red}{then we can compute an implied disk 
opening angle by simple asking what proportion of the luminosity of 
the central source must be intercepted
by the disk surface. This }implied disc opening angle is
$\simeq$40\degree. However, in this high luminosity regime, radiation
driving may also help to increase the \textcolor{red}{ efficiency of conversion 
from radiative to kinetic energy and therefore mean that a smaller
opening angle is required.}
